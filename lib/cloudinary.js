...
const getThumbnailUrl = (url) => {
  const transformations = "c_scale,f_webp,w_300";
...
const getLQIPUrl = (url) => {
  const transformations = "c_scale,f_webp,q_1,w_10";
...
export const getFiles = async (maxResults, nextCursor, text, order) => {
  ...
  const newResources = await Promise.all(
    resources.map(async (resource) => {
      ...
      const lqipUrl = getLQIPUrl(resource.secure_url);
  ...
  return { resources: newResources, next_cursor };
};
